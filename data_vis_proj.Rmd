...
Based on very strong correlation between G1, G2 and G3, which correspond to first period, second period and final grade respectively, we can merge them into one average grade and continue the study considering average_grade as the dependent variable.
...
made that in both geographical regions, male students perform better than female students on average. An interesting note is that the 
performance of males living in rural areas is almost equal to that of female students living in urban areas on average. Since the notches in the box plot do not overlap, we can conclude, with 95% confidence, that the actual medians do differ. Finally, we do not reject our null hypothesis stating that urban students perform better than rural students.
...
Firstly the figure implies that there not much evidence of parents not having any education. The most common grades for students whose parents have primary education varies between 7 to 10, which is a bad result overall. Common grades for students whose parents have 5th to 9th-grade education vary between 10 to 15, which is a decent result. There is a similar pattern for students whose parents have only secondary or only higher education compared to the previous group. In conclusion, we do not reject the null hypothesis, which states that parents' education and extra help in studies positively affect student performance. However, parents with at least 5th to 9th-grade education already affect their child's academic performance positively, and there is not much difference for parents with higher education than the one mentioned.
...
The bar plot identifies that 


Hypothesis 6: Intense alcohol consumption strongly decreases students’ performance based on gender

```{r cars7}

agg_data <- aggregate(avrg_G ~ Dalc, data = data, FUN=mean)

ggplot(agg_data, aes(x = Dalc, y = round(avrg_G,2))) + geom_line(size=2, color = 'orange') + geom_point(size=3, color = 'red') + labs(x = 'Daily Alcohol Consumption', y = "Average Grade of Exams") + geom_text(aes(label=round(avrg_G,2)), position=position_dodge(width=0.9), vjust=-1, size = 4, color = "black") + theme() + scale_fill_discrete(labels=c('Female', 'Male')) + guides(fill=guide_legend("Gender")) + theme_bw() + ggtitle('Average grade of students based on daily alcohol consumption') + ylim(8,12.5) + scale_x_continuous(breaks=c(1,2,3,4,5), labels=c("Very low","Low","Medium","High", "Very high"))

```
Hypothesis 7: Being in a romantic relationships harms students’ average grade based on gender.

```{r cars8}
agg_data <- aggregate(avrg_G ~ romantic + sex, data = data, FUN=mean)

ggplot(agg_data, aes(x = romantic, y = round(avrg_G,2), fill = sex)) + geom_bar(stat='identity', position = 'dodge') + labs(x = 'In a romantic relationship', y = "Average Grade of Exams") + geom_text(aes(label=round(avrg_G,2)), position=position_dodge(width=0.9), vjust=-0.3, size = 4, color = "black") + scale_fill_manual(values=c('salmon','darkturquoise'), labels=c('Female', 'Male')) + guides(fill=guide_legend("Gender")) + theme_few() + ggtitle('Average grade of students based on romantic relationship') + ylim(0,12.5) + theme(panel.background = element_rect(fill = "#FFC4C47D")) +  scale_x_discrete(breaks=c("no","yes"), labels=c("No", "Yes"))
...
Hypothesis 8: It is significantly better to live alone as a student in terms of academic performance.
...
Hypothesis 9: Students who do not hang out with friends are expected to have better performance.
...
```
Hypothesis 10: Many absences lead to many failures.

```{r cars11}
correlation <- cor(data$failures, data$absences)

ggplot(data, aes(x = failures, y = absences)) + geom_point() + geom_jitter(height = 0.1, width = 0.1) + geom_smooth(formula=y~x,method = 'lm', se=FALSE) + theme_classic() + labs(x = 'Failures', y = "Absences", caption = paste0('Correlation coefficient ', round(correlation,3))) + ggtitle('Student absences vs failures')

```

Hypothesis 11: Students studying less are expected to have significantly lower average grade. 

```{r cars12}
devtools::install_github('cttobin/ggthemr')
ggplot(data = data, aes(x = studytime, y=avrg_G, group = studytime, fill = factor(studytime))) + geom_violin() + geom_boxplot(width = 0.3) + ggtitle('The distribution boxplot of average grade based on study time of the students') + labs(x='Study hours (weekly)', y = 'Average grade') + theme_hc() +  theme(legend.position = "none") + scale_fill_brewer(palette='Pastel2') + scale_x_discrete(limits=c("1","2","3","4"),labels = c("<2","2-5","5-10",">10"))
```


```{r}

 ggplot(data, aes(x=data[ ,c(input$VarX)])) + geom_bar(fill = 'darkorange') + theme_gdocs() + theme(legend.position = "none")
```
